...
function EmployeeDashboard({ user, onLogout }) {
        ...
        const fetchData = async () => {
            ...
            try {
                if (view === 'timesheet' || view === 'payroll') {
                    ...
                    if (view === 'timesheet') {
                        const data = await apiFetchTimesheet(user.id, yearMonth);
                        setTimesheetData(data);
                    } else {
                        const data = await apiFetchPayroll(user.id, yearMonth);
                        setPayrollData(data);
                    }
    ...
    const handleRegisterBiometric = async () => {
        try {
            const options = await apiGenerateRegistrationOptions();
            const registrationResult = await startRegistration(options);
            const { verified } = await apiVerifyRegistration(registrationResult);
            if (verified) {
            ...
            const errorMessage = err.message || 'Thiết bị của bạn không hỗ trợ hoặc đã có lỗi xảy ra.';
            ...
            <header className="bg-white shadow-sm">
                <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row sm:justify-between items-start sm:items-center">
                    ...
                    <div className="flex flex-wrap gap-2">
                        <button onClick={handleRegisterBiometric} className="px-4 py-2 text-sm font-medium text-green-600 bg-green-100 rounded-md hover:bg-green-200">
                            Kích hoạt Face ID / Vân tay
                        </button>
                ...
                <div className="bg-white rounded-lg shadow p-4 sm:p-6">
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                         <h2 className="text-2xl font-bold text-gray-800 flex-shrink-0">
                            {view === 'timesheet' ? 'Bảng chấm công' : view === 'payroll' ? 'Bảng lương' : 'Ngày nghỉ phép'}
                         </h2>
                        <div className="flex flex-wrap items-center gap-2">
                            <div className="flex items-center gap-2">
                                ...
                                {view !== 'holiday' && (<select value={currentMonth} onChange={(e) => setCurrentMonth(parseInt(e.target.value))} className="px-3 py-2 border border-gray-300 rounded-md shadow-sm w-36">{months.map(month => <option key={month.value} value={month.value}>{month.name}</option>)}</select>)}
                            </div>
