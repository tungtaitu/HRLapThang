function showOverTimeDetail(objSender){
   var trDetail = $(objSender).parent().parent().next();
    ...
    if(jQuery.trim($(tdDetail).html())==""){
        ...
        getOverTimeDetail(tdDetail,vou_no);        
    }
...
//批次審核的加班明細
function showMuch_OverTimeDetail(objSender){
    var trDetail = $(objSender).parent().parent().next();
    var tdDetail = $(objSender).parent().parent().next().find("td:eq(0)");
    //loading data
...
function getOverTimeDetail(objSender,vou_no){
    var objServerVar;
	...
	$.ajax(
		{
			url:"../../Pub/RemoteMethods/Detail.aspx",
			type:"post",
			data:"EventName=getOverTimeDetail&vou_no="+vou_no,
			dataType:"text",
			timeout:100000,
			error:function(err){
				objServerVar = {
