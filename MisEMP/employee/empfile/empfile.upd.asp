...
IF REQUEST("ACT")="EMPEDIT"  THEN
	...
	if TRIM(OUTDAT)<>"" THEN 
		SQL="UPDATE EMPFILE SET INDAT='"& INDAT &"',    "&_
			...
			"MARRYED='"& MARRYED &"' , SCHOOL='"& SCHOOL &"' , grps='"& grps &"', studyjob=N'"& studyjob &"' "&_
...
ELSEIF REQUEST("ACT")="EMPADDNEW" THEN
	SQL="INSERT INTO EMPFILE (EMPID, INDAT, TX, EMPNAM_CN, EMPNAM_VN, COUNTRY, "&_
		"BYY, BMM, BDD, AGES, SEX, PERSONID, BHDAT, PASSPORTNO, PDUEDATE, VISANO, VDUEDATE, PHONE, "&_
		"MOBILEPHONE, HOMEADDR, EMAIL, GTDAT, MEMO, MDTM, MUSER,marryed, SCHOOL ) VALUES ( "&_
		"'"& EMPID &"', '"& INDAT &"', '0', '"& EMPNAM_CN &"', "&_
		...
		"'"& VISANO &"', '"& VDUEDATE &"', '"& PHONE &"', '"& MOBILEPHONE &"' , N'"& HOMEADDR &"' ,'"& EMAIL &"' , "&_
 		"'"& GTDAT &"', '"& MEMO &"', GETDATE() , '"& SESSION("NETUSER") &"', '"& marryed &"', '"& SCHOOL &"' ) "
 	conn.execute(sql)
