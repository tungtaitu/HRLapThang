...
sql=" select  b.groupid, b.gstr, b.empnam_cn,  convert(char(10), a.mdtm, 111) as keyindat , "&_
	"convert(char(10), sdat, 111) as sdate, convert(char(10), edat, 111) as edate , convert(char(10), a.indat, 111) as indate,  a.* from "&_
	"( select * from empHTdata  ) a  "&_
	...
	for i = 1 to TotalPage
		for j = 1 to PageRec
			if not rs.EOF then	
				...
				tmpRec(i, j, 6) = trim(rs("indate"))
				tmpRec(i, j, 7) = trim(rs("sdate"))
				tmpRec(i, j, 8) = trim(rs("edate"))
				tmpRec(i, j, 9) = trim(rs("memo"))				
				tmpRec(i, j, 10) = trim(rs("aid"))
				tmpRec(i, j, 11) = trim(rs("country"))
				rs.movenext
			else
				exit for			
			end if 
...
<table width="460" border="0" cellspacing="0" cellpadding="0">
	...
	<tr height=40 >
		<td width=150 align=center valign=middle>
			...
			<a href="empbe04.asp" target="_parent">幹部合同資料新增</a>
		</td>
		<td width=180 align=center>
			
			<font color="Brown"><b>幹部合同資料異動查詢</b></font>
		</td>
		<td></td>
	</tr>
</table>
...
<fieldset style="margin:0;padding:0;width=580"><legend><font class=txt9>資料查詢</font></legend>
	<table width=570 class=txt9 border="0" cellspacing="0" cellpadding="0" >	
		<tr height=35>
			...
			<td width=80 align=right>合同到期日:</td>
		 	<td width=180>
...
<table width=500  ><tr><td >
	<table width=480 class=txt9>
		...
		<tr>
			...
			<td>
				...
				<input type=hidden name=aid value="<%=tmpRec(CurrentPage, CurrentRow, 10)%>">
			</td>	
			<td>
				<input name=empname size=15 class=readonly readonly value="<%=tmpRec(CurrentPage, CurrentRow, 2)%>" >
				<input type=hidden name=country  value="<%=tmpRec(CurrentPage, CurrentRow, 11)%>" >
			</td>	
			<td>
				<input name=indate size=11 class=readonly readonly  value="<%=tmpRec(CurrentPage, CurrentRow, 6)%>" >
			</td>	
			<td>
			...
			<td>
				<input name=memo size=25 class=inputbox onblur="visanochg(<%=CurrentRow-1%>)" value="<%=tmpRec(CurrentPage, CurrentRow, 9)%>">
			</td>	
		</tr>
		<%next%>
	</table>	
	...
	<table width=450 align=center>
		...
		<tr >
			<td align=center>
				<input type=button  name=btm class=button value="確   認 修 改" onclick="go()" onkeydown="go()">
				<input type=button  name=btm class=button value="取   消"  onclick="JavaScript:window.location.reload()">
			</td>
		</tr>	
	</table>	
