		...
		<TABLE WIDTH=500 CLASS=TXT9>
	 		<TR>
				...
				<TD width=80>績效年月:</TD>
	 			...
	 			<td>
	 				<input type=button name=btm class=button value="(S)查 詢"   onkeydown="SCHDATA()" onclick="SCHDATA()">
	 			</td> 	 			
	 		</TR>
	 	</TABLE>		
	 	<hr size=0	style='border: 1px dotted #999999;' align=left width=590> 
	 	<TABLE WIDTH=590 CLASS=TXT9 BGCOLOR="#CCCCCC" BORDER=0 border="1" cellspacing="1" CLASS=TXT9 ALIGN=CENTER>
	 		<TR BGCOLOR="#FFF278">
				<TD WIDTH=50 HEIGHT=22 ALIGN=CENTER>DEL</TD>
				<TD WIDTH=30 HEIGHT=22 ALIGN=CENTER>STT</TD>
				<TD WIDTH=60 HEIGHT=22 ALIGN=CENTER>廠別</TD>
			...
			<%if CurrentRow>1 and ( tmpRec(CurrentPage, CurrentRow, 10)<>tmpRec(CurrentPage, CurrentRow-1, 10) or  tmpRec(CurrentPage, CurrentRow, 4)<>tmpRec(CurrentPage, CurrentRow-1, 4) )then %>
	 		...
	 		<TR BGCOLOR="#FFFFFF" >
	 			...
	 			<TD ALIGN=CENTER>
	 				<select name=jxwhsno class="txt8" style="width:60">
	 				<option value="">---</option>
	 				<%sqlx="select *from basicCode where func='whsno' order by sys_type " 
	 				  set rst=conn.execute(Sqlx)
	 				  while not rst.eof 
	 				%>	
	 				<option value="<%=rst("sys_type")%>" <%if tmpRec(CurrentPage, CurrentRow, 13)=rst("sys_type") then%>selected<%end if%>><%=rst("sys_type")%>-<%=rst("sys_value")%></option>
	 				<%rst.movenext
	 				wend
	 				rst.close
	 				set rst=nothing
	 			...
	 			<TD ALIGN=CENTER>
	 				<INPUT NAME=groupid CLASS="readonly2" READONLY SIZE=5 value="<%=tmpRec(CurrentPage, CurrentRow, 10)%>">
	 			</TD>
	 			<TD ALIGN=CENTER>
	 				<INPUT NAME=shift  CLASS="readonly2" READONLY  SIZE=3  value="<%=tmpRec(CurrentPage, CurrentRow, 4)%>" style="text-align:center">
	 			</TD>
	 			<TD ALIGN=CENTER>
	 				<INPUT NAME=zstr  CLASS="readonly2" READONLY  SIZE=8  value="<%=tmpRec(CurrentPage, CurrentRow, 12)%>" style="text-align:center">
	 			...
	 			<TD ALIGN=CENTER>
	 				<INPUT NAME=DESCP CLASS=INPUTBOX SIZE=20  value="<%=tmpRec(CurrentPage, CurrentRow, 6)%>" >
	 			</TD>
	 			<TD ALIGN=CENTER>
