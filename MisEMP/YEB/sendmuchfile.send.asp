  ...
  ' intCount = mySmartUpload.Save("UpdData")
   ...
   '  **********************
      ...
      '  ******************************************
         ...
         sql="select * from wbphotos where filename='"& file.FileName  &"' "
         'response.write sql 
         'response.end 
         set rs=conn.execute(Sql)
         if  rs.eof then 
         	sql="insert into wbphotos ( filename ) values ('"& file.FileName &"' ) "
         	response.write sql 
         	conn.execute(Sql)
         end if 
         set rs=nothing
...
<% ELSE
		...
		set conn=nothing
		response.clear 
		response.write "Sorry,不支援中文檔名<BR>"
		response.write "<a href='vbscript:window.close()'>關閉</a>　　"
		response.write "<a href='send.muchfile.asp'>重新傳送</a>"
		response.end 
