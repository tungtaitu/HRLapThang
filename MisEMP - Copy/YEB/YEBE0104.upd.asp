...
sysid = request("sysid")
if flag="del" then
	sqld="update wbempfile set status='D' ,mdtm=getdate(), muser='"& session("netuser") &"' where wbid='"& wbid &"'  and aid='"& sysid &"' "
	conn.execute(Sqld)
	response.redirect "yebe0104.fore.asp?EMPID="& wbid
	response.end
end if
...
if not rs.eof then
	sql="update wbempfile set wbname_cn=N'"&nam_cn&"',wbname_vn=N'"&nam_vn&"',indat="&indat&", "&_
		"job=N'"&job&"',country='"&country&"',yy='"&yy&"',mm='"&mm&"',dd='"&dd&"',age='"&age&"', "&_
		...
		"outmemo=N'"&outmemo&"',flag='"&flag&"',addr=N'"&addr&"',mdtm=getdate(), "&_
		"muser='"&session("netuser")&"', filename='"& filename &"'  "&_
		"where wbid='"&wbid&"'"
	...
	sql="insert into wbempfile (loai,wbwhsno,wbid,wbname_cn,wbname_vn,indat,yy,mm,dd,age,sex,personid,phone,mobile, "&_
		...
		"'"&wblosi&"','"&whsno&"','"&wbid&"',N'"&nam_cn&"',N'"&nam_vn&"',"&indat&",'"&yy&"','"&mm&"','"&dd&"', "&_
		"'"&age&"','"&sex&"','"&personid&"','"&phine&"','"&mobile&"',N'"&fac&"','"&lorry&"','"&soxe&"', "&_
		"N'"&job&"',N'"&addr&"',N'"&wbmemo&"',getdate(),'"&session("netuser")&"','"&filename&"' )  "
end if
response.write sql
conn.execute(sql)
