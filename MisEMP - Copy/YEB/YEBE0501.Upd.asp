...
for i = 1 to gTotalPage 
	for j = 1 to PageRec 
		...
		if amt="" then amt=0
		'response.write studyName
		
		if tmpRec(i, j , 0) = "del" then		
			sql="update studyPlan set status='D' where ssno='"& ssno &"' "	
			'response.write sql
			conn.execute(sql)	
			X = X+1 
			y = y & "年度:" & planyear &" 編號:" & ssno & " 課程名稱:" & studyName &" DEL(Da Huy)<BR>"
		else
			if trim(ssno)<>"" then 
			 	sql="update studyPlan set studyName=N'"& studyName &"', T1='"&T1&"', T2='"&T2&"' ,  "&_
			 		...
			 		"memo='"&memo&"' , mdtm=getdate(), muser='"& session("NETUSER") &"' where ssno='"& ssno &"' and aid='"& aid &"' " 
			 	'response.write sql &"<BR>"	
			 	conn.execute(Sql)
			 	X = X+1 
			 	y = y & "年度:" & planyear &" 編號:" & ssno & " 課程名稱:" & studyName &" 異動成功(OK)<BR>"
			 else
			 	if trim(studyName)<>"" then 
			 		...
			 		set rst=nothing 
			 		sql="insert into studyPlan (yy, ssno,studyName,t1, t2, t3, t4, t5, t6,t7, t8, t9, t10,t11,t12, amt, dm, pcnt, hhour, nw, memo,muser ) values ( "&_
			 			"'"& planyear &"', '"& ssno &"', N'"& studyName &"', '"& T1 &"','"& T2 &"','"& T3 &"','"& T4 &"','"& T5 &"','"& T6 &"', "&_
			 			"'"& T7 &"','"& T8 &"','"& T9 &"','"& T10 &"','"& T11 &"','"& T12 &"','"& amt &"','"& dm &"','"& pcnt &"','"& hhour &"', "&_
			 			"'"& nw &"',N'"& memo &"','"& session("NETUSER") &"' )" 
			 		'response.write sql &"<BR>" 	
			 		conn.execute(Sql)
			 		y = y & "年度:" & planyear &" 編號:" & ssno & " 課程名稱:" & studyName &" 處理成功(OK)<BR>"
			 		X = X+1
			 	end if 
			 end if 		
		end if 		
...
if x=0 then 
	...
	response.write "<tr><td align=center><BR><BR><a href='yebe0501.ASP?totalpage=0&planyear='"& planyear &"''><U><FONT COLOR=BLUE>回主畫面</FONT></U></A></td></tr></table>"
	...
	if  err.number=0  then 
		...
		response.write "<table width=550 class=font9><tr><td align=center ><table width=600 class=font9><tr><td>"
		response.write "<font class=txt12 color=red>資料處理成功OK(SUCCESS)!!</font><BR><BR>"
		response.write y
		response.write "</td></tr>"
		response.write "<tr><td align=center><BR><BR><a href='yebe0501.Fore.ASP?totalpage=0&planyear="& planyear &"'><U><FONT COLOR=BLUE>回主畫面</FONT></U></A></td></tr></table>"
		response.write "</td></tr></table>"
		...
		response.write "</td></tr>"
		response.write "<tr><td><BR><BR><a href='yebe0501.ASP?totalpage=0&planyear='"& planyear &"''><U><FONT COLOR=BLUE>回主畫面</FONT></U></A></td></tr></table>"
		response.write "</td></tr></table>"	
		Response.End 
	end if 	 
end if 	
