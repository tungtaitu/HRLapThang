	...
	for j = 1 to PageRec 
		...
		if tmpRec(i, j , 0) = "del" then		
			...
			if trim(tmpRec(i, j, 0))="upd" then 
				...
				yymm=trim(tmpRec(i, j, 25))
				sql="select * from BempJ where yymm='"& yymm &"' and empid='"& empid &"'   "
				'response.write sql 
				...
				if rds.eof then 
					...
					sqla="update  BempJ set whsno='"& whsno &"' , job='"& job &"' , memo ='"& memo &"' , mdtm=getdate(), muser='"& session("netUser") &"' "&_
				...
				'tmpRec(i, j, 27) = rsTmp("sys_value")
				x=x+1
				y=y& X &". "&trim(tmpRec(i, j, 13))&" "&empid&" "&empinfo& "<BR>異動日期: " & yymm &"<br>異動內容: " & trim(tmpRec(i, j, 24))&trim(tmpRec(i, j, 15))&" → " & trim(tmpRec(i, j, 6))&tmpRec(i, j, 27) &"<BR>異動說明:<BR>" & trim(memo) &"<hr size=0	style='border: 1px dotted #999999;' align=left >"
			end if	
		end if 		
	next
next 
...
if x=0 then 
	...
	if  conn.errors.count=0  then 
		...
		response.write y
		response.write "</td></tr>"
		response.write "<tr><td align=center><BR><BR><a href='YEBBB0102.ASP'><U><FONT COLOR=BLUE>回主畫面</FONT></U></A></td></tr></table>"
		response.write "</td></tr></table>"
		response.end 
		...
		response.write "<table width=500 class=font9><tr><td align=center ><table width=350 class=font9><tr><td>"	
		response.write "</td></tr>"
		response.write "<tr><td><BR><BR><a href='YEBBB0102.ASP'><U><FONT COLOR=BLUE>回主畫面</FONT></U></A></td></tr></table>"
		response.write "</td></tr></table>"	
		Response.End 
	end if 	 
end if 	
