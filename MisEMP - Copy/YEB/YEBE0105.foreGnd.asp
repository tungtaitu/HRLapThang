...
sql="select isnull(b.incustsname,'') incustsname , a.* from "&_
	"(select convert(char(10),indat,111) indate, convert(char(10),outdat,111) outdate, * from wbempfile "&_
	"where wbid='"&wbid&"' ) a  "&_
	"left join (select incustid, incustsname from [yfymis].dbo.ydbscust ) b on b.incustid = a.fac " 
...
<table width="460" border="0" cellspacing="0" cellpadding="0">
	<tr><TD  >
		...
		1.4外包資料查詢K.Tra tu lieu thau ngoai
		</TD>
	</tr>
</table>
<hr size=0	style='border: 1px dotted #999999;' align=left width=500 > 
<TABLE WIDTH=520 CLASS=txt BORDER=0 cellspacing="2" cellpadding="2" >
	<TR height=35 >
		...
		<td    align=center valign=top  nowrap  rowspan="4"  colspan=2>
			<img src="pic/wb/<%=wbid%>.jpg"  border=1 width=98 height=120  >
		</td>	
	</tr>		
	<TR height=35 >
	...
	<TR height=35 >	
		...
		<TD  valign=top><INPUT NAME="EMPID" SIZE=12 CLASS=readonly readonly maxlength=5 value="<%=wbid%>"></TD>
	</tr>
	<TR height=35 > 
		<TD   align=right height=25>到職日<BR><font class=txt8>NVX</font></TD>
		<TD valign=top><INPUT NAME=indat SIZE=12 CLASS=readonly readonly value="<%=indate%>" onblur="date_change(1)"></TD> 
	</tr>
	<TR height=35 > 
		<TD   align=right >職等<br><font class=txt8>Chuc vu</font></TD>
		<TD  valign=top>			
			<select name=JOB  class=txt8 disabled > 
				<option value="">----</option>
				<option value="TX" <%if job="TX" then %>selected<%end if%>>司機(TX)</option>
				<option value="PX" <%if job="PX" then %>selected<%end if%>>捆工(PX)</option>
			</SELECT>
			<%SET RST=NOTHING %>
		</TD>
		<TD   align=right>國籍<br><font class=txt8>Quoc Tich</font></TD>
		<TD  valign=top >
			<select name=country  class=font9 style='width:75' disabled  >
	...
	<TR height=35>
		...
		<TD valign=top >
			<INPUT NAME=nam_cn SIZE=22 CLASS=readonly readonly   value="<%=wbname_cn%>">
		</TD> 
		<TD   align=right >姓名(越)<BR><font class=txt8>Ho Ten(viet)</font></TD>
		<TD valign=top  ><INPUT NAME=nam_vn SIZE=30 CLASS=readonly readonly  value="<%=wbname_vn%>"></TD>
	</TR>
	<TR height=35 >
		<TD   align=right >出生日期<br><font class=txt8>Ngay Sinh</font></TD>
		<TD colspan=3 valign=top>
		<INPUT NAME=BYY SIZE=5 CLASS=readonly readonly  MAXLENGTH=4 ONBLUR="CHKVALUE(1)" value="<%=yy%>"> &nbsp;年<font class=txt8>Năm</font>&nbsp;
		<INPUT NAME=BMM SIZE=5 CLASS=readonly readonly  MAXLENGTH=2 ONBLUR="CHKVALUE(2)" value="<%=mm%>"> &nbsp;月<font class=txt8>Tháng</font>&nbsp;
		<INPUT NAME=BDD SIZE=5 CLASS=readonly readonly  MAXLENGTH=2 ONBLUR="CHKVALUE(3)" value="<%=dd%>"> &nbsp;日<font class=txt8>Ngày</font>&nbsp;&nbsp;&nbsp;				
		Age(Thoi)<input name=ages size=3 class=readonly readonly value="<%=ages%>"> 
		...
		<td valign=top><input name=personID size=30 class=readonly readonly value="<%=personid%>"></td>		
	</tr>
	<tr height=35>
		<td  align=right >卡片號碼<BR><font class=txt8>Card No</font></td>
		<Td><INPUT NAME=cardno SIZE=22 CLASS=readonly readonly  value="<%=cardno%>"></td>
	</tr>
	<Tr>
	...
	<tr height=35>
		...
		<td valign=top>
			...
			<%else%>
				<input name=fac size=22 class=readonly readonly value="<%=fac%>"    >
			<%end if%>
		...
		<td valign=top>
			...
			<input name=soxe size=9 class=readonly readonly  value="<%=soxe%>" >			
		</td>
	</tr>		
	<tr >
		<td   align=right height=25 >聯絡電話<BR><font class=txt8>Đ.T</font></td>
		<td valign=top><input name=phone size=22 class=readonly readonly  value="<%=phone%>"></td>
		<td   align=right >手機<BR><font class=txt8>ĐTDD</font></td>
		<td valign=top><input name=mobile size=15 class=readonly readonly value="<%=mobile%>"></td>
	</tr> 
	<tr height=35>
		<td nowrap align=right height=25 >聯絡地址<BR><font class=txt8>Địa chi<BR></td>
		<td colspan=3 valign=top><input name=homeaddr size=65 class=readonly readonly value="<%=addr%>" ></td>
	</tr>
	<tr height=35>
		<td nowrap align=right height=25 >其他說明<BR><font class=txt8>Ghi Chu</font></td>
		<td valign=top colspan=3><input name=memo size=65 class=readonly readonly value="<%=wbmemo%>"></td>		
	</tr>
	<Tr>
		<Td colspan=4><hr size=0 style='border: 1px dotted #999999;'  ></td>
	</tr> 
	<tr height=35>
		...
		<td valign=top ><input name=outdat size=11 class=readonly readonly value="<%=outdate%>"  onblur="date_change(2)"></td>
		<td nowrap colspan=2 >
			<input type=checkbox name=ff   <%if flag="Y" then%>checked<%end if%> disabled > 已還識別證(TRA THE)
	...
	<tr height=35>
		...
		<td valign=top colspan=3><input name=outmemo size=65 class=readonly readonly value="<%=outmemo%>"></td>		
	</tr>	
</table>
<hr size=0	style='border: 1px dotted #999999;' align=left width=500>
<TABLE WIDTH=460>
