		...
		if trim(tmpRec(i, j, 1))<>"" then
			...
			IF RS.EOF THEN
				SQL1="INSERT INTO EMPDSALARY ( WHSNO,COUNTRY, EMPID, indat, outdat, GROUPID,JOB,BB,CV,PHU,NN,KT,MT,TTKH,QC,TNKH, "&_
					 ...
					 "ZHUANM, XIANM , memo , userip, DKM  ) VALUES ( "&_
					 ...
					 "'"& ZHUANM &"', '"& XIANM &"', '"& memostr &"','"& session("vnlogIP") &"', '"& DKM &"'     ) "
			...
			ELSE
				SQL2="update EMPDSALARY set whsno = '"& trim(tmpRec(i, j, 7)) &"', GROUPID='"& trim(tmpRec(i, j, 9)) &"', JOB='"& trim(tmpRec(i, j, 6))  &"', "&_
					 ...
					 "mdtm=getdate(), muser='"& session("NETUSER") &"' , userip='"& session("vnlogIP") &"' , DKM='"& DKM &"' "&_
...
if err.number = 0 then
	conn.CommitTrans
	Set Session("empfilesalaryCN") = Nothing
	Set conn = Nothing
%>	<SCRIPT LANGUAGE=VBSCRIPT>
		ALERT "資料處理成功!!"&"<%=X%>"&"筆"&chr(13)&"DATA CommitTrans Success !!"
		OPEN "empsalaryCN.asp" , "_self"
	</script>
<%
ELSE
	conn.RollbackTrans
