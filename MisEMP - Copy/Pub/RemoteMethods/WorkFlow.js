/* show workflow list detail */
function showFlow(objSender){
    ...
    if(jQuery.trim($(tdDetail).html())==""){
        var FlowID = $(objSender).parent().find(":hidden:eq(0)").val();            
        getDetail(tdDetail,FlowID);
    }
...
function getDetail(objSender,FlowID){
    var objServerVar;
	...
	$.ajax(
		{
			url:"../../../Pub/RemoteMethods/WorkFlow.aspx",
			type:"post",
			data:"EventName=getDetail&FlowID="+FlowID,
			dataType:"text",
			timeout:100000,
			error:function(err){
				objServerVar = {
