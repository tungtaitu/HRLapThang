function showMissCardDetail(objSender){
...
//批次審核的加班明細
function showMuch_MissCardDetail(objSender){
    var trDetail = $(objSender).parent().parent().next();
    var tdDetail = $(objSender).parent().parent().next().find("td:eq(0)");
    //loading data
...
function getMissCardDetail(objSender,vou_no){
    var objServerVar;
	...
	$.ajax(
		{
			url:"../../Pub/RemoteMethods/Detail_MissCard.aspx",
			type:"post",
			data:"EventName=getMissCardDetail&vou_no="+vou_no,
			dataType:"text",
			timeout:100000,
			error:function(err){
				objServerVar = {
