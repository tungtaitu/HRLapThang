...
for i = 1 to TotalPage
	for j = 1 to PageRec
		...
		if trim(tmpRec(i, j, 1))<>"" then 
			...
			IF tmpRec(i, j, 36)="" THEN	JIAB=0 ELSE	JIAB=CDBL(tmpRec(i, j, 36))
			...
			IF RS.EOF THEN
				...
				SQL1="INSERT INTO EMPDSALARY ( WHSNO, Country, EMPID, indat, outdat , GROUPID,JOB,BB,CV,PHU,NN,KT,MT,TTKH,QC,TNKH, "&_
					 ...
					 "JISHU, LZBZJ, BZKM, KTAXM, dm, ZHUANM, XIANM , userIP , memo, sgkm , btien , h12, h12m) VALUES ( "&_
					 ...
					 "'"& DM &"' ,'"& ZHUANM &"', '"& XIANM &"', '"& session("vnlogIP") &"' ,'"& MEMOSTR &"','"& sgkm &"' ,'"& btien &"' , '"& h12 &"','"& h12m &"') "
