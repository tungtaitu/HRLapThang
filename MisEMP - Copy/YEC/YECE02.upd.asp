	...
	for j = 1 to PageRec
		...
		if trim(tmpRec(i, j, 1))<>"" then
			...
			if tmpRec(i, j, 4) = "VN" then 
				IF TRIM(tmpRec(i, j, 69))="" THEN   '稅金
					KTAXM=0
				ELSE
					KTAXM=CDBL(tmpRec(i, j, 69))
				END IF 
			else	
			...
			IF TRIM(tmpRec(i, j, 51))="" THEN
				H1M=0
			ELSE
				H1M=CDBL(tmpRec(i, j, 51))
			END IF

			IF TRIM(tmpRec(i, j, 52))="" THEN
				H2M=0
			ELSE
				H2M=CDBL(tmpRec(i, j, 52))
			END IF

			IF TRIM(tmpRec(i, j, 53))="" THEN
				H3M=0
			ELSE
				H3M=CDBL(tmpRec(i, j, 53))
			END IF
			...
			MEMOSTR = trim(REPLACE(tmpRec(i,j,70), "'", "" ))
			...
			IF RS.EOF THEN
				...
				SQL1="INSERT INTO EMPDSALARY ( WHSNO, Country, EMPID, indat, outdat , GROUPID,JOB,BB,CV,PHU,NN,KT,MT,TTKH,QC,TNKH, "&_
					 ...
					 "JISHU, LZBZJ, BZKM, KTAXM, dm, ZHUANM, XIANM , userIP , memo ) VALUES ( "&_
					 ...
					 "'"& DM &"' ,'"& ZHUANM &"', '"& XIANM &"', '"& session("vnlogIP") &"' ,'"& MEMOSTR &"'   ) "
