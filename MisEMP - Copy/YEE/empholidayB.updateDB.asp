		...
		for j = 1 to  pagerec     
			'response.write tmpRec(i, j, 0) &"<BR>" 		     
		    	...
		    	'RESPONSE.WRITE SQLA &"<br>"		  
					sql="exec sp_insempHolidaytowktime '"&TRIM(tmpRec(i, j, 1))&"','"&REPLACE( TRIM(tmpRec(i, j, 17)), "/","" )&"' ,'"&TRIM(tmpRec(i, j, 22))&"' ,'"&TRIM(tmpRec(i, j, 23))&"' ,'D' "
					conn.execute(sql)  					 
				end if 	
	    next 
	Next 
		...
		for j = 1 to  pagerec     
		    ...
		    if  tmpRec(i, j, 0) = "del"  then 
		    	...
		    	X = X+1        
					...
					'RESPONSE.WRITE SQLA &"<br>"
					'CONN.EXECUTE(SQLA) 
					sql="exec sp_insempHolidaytowktime '"&TRIM(tmpRec(i, j, 1))&"','"&REPLACE( TRIM(tmpRec(i, j, 17)), "/", "" ) &"' , " &_
							"'"&TRIM(tmpRec(i, j, 22))&"' ,'"&TRIM(tmpRec(i, j, 23))&"' ,'D' "
					conn.execute(sql)  					 
