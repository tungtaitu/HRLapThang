...
if sts="NALL" then 
	...
	FOR I = 1 TO PageRec 
		...
		IF TRIM(DESCP)<>"" AND TRIM(PER)<>"" AND TRIM(HESO)<>"" AND TRIM(HXSL)<>"" THEN   
			...
			if rds.eof then 
				SQL="INSERT INTO YFYMJIXO (jxwhsno,JXYM,SALARYYM,GROUPID,SHIFT, zuno, STT,DESCP,HXSL,HESO,PER) VALUES (  "&_
					...
					"N'"& DESCP &"' , '"& HXSL &"' ,'"& HESO &"' ,'"& PER &"' ) " 
				CONN.EXECUTE(SQL)	
				response.write "A=" & sql &"<BR>"
			else
				sql="update YFYMJIXO set descp=N'"& descp  &"' ,   "&_
...
if conn.Errors.Count = 0 then 
	...
	<SCRIPT LANGUAGE=VBSCRIPT>
		...
		OPEN "YFYEMPJX.copydata.asp?Njxym="&"<%=Njxym%>"&"&G1="&"<%=G1%>"&"&z1="&"<%=z1%>"&"&mywhsno="&"<%=mywhsno%>" , "_self" 
	</script>	
<%	
	else
%>	<SCRIPT LANGUAGE=VBSCRIPT>
